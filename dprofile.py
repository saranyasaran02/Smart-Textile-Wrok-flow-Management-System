...
id = form.getvalue("id")
x = """select * from dealer where id="%s" """ % (id)
cur.execute(x)
res = cur.fetchall()
for i in res:
    print("""
    ...
    print("""
           <body>
               ...
               <div class="content" id="content">
                     <h2 class="page-title">Raw Material</h2>
                     ...
                     <div class="profile-card">
                         ...
                         </div>"""%(i[0],i[0],i[0],i[0],i[8],i[1],i[2],i[3],i[4],i[5]))
    print("""
...
if editsubmit != None:
    q = """update dealer set full_Name='%s', email='%s', phone='%s' ,location='%s' where Id='%s' """ % (
    ...
    print("""
           ...
           window.location.href="dprofile.py?id=%s"
           </script>
    """ % (id))
...
if isubmit != None:
    ...
    x = """UPDATE dealer set image='%s' where Id='%s' """ % (fn, admin_id)
    cur.execute(x)
    con.commit()
...
if psubmit:
    ...
    ch = "SELECT Password FROM dealer WHERE Id=%s"
    cur.execute(ch, (chid,))
    result = cur.fetchone()
    ...
    if result:
        ...
        if currentpass != stored_password:
            ...
            update_query = "UPDATE dealer SET Password=%s WHERE Id=%s"

             ...
             <head>
                 ...
                 <style>
                     ...
                     .dealer-table {
                         ...
                         width: 100%;
                         background-color: white;
                         min-width: 1400px; /* Ensures table doesn't get too compressed */
                     }
...
for i in res:
    ...
    """ % (i[1], i[2], i[3], i[4], i[5], i[6], i[7],i[8],i[12]))
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary mb-4" data-bs-dismiss="modal">Cancel</button>
            <input type="submit" class="btn btn-action mb-4" value="Confirm Approval" name="update">
            </form>
          </div>

        </div>
      </div>
    </div>"""%(dealer_id,dealer_id,i[1],user_id,password,i[3],i[1],dealer_id))
    print("""
    ...
    <i class="fa-solid fa-circle-xmark"></i> Reject</button></td>
          ...
          <div class="modal-footer">
            ...
            <input type="submit" class="btn btn-action mt-4" value="Reject" name="reject">
            </form>
          </div>
...
if Submit is not None and dealer_id is not None:

    q = """update dealer set username='%s', Password='%s', status="approved" where Id='%s' """ % (
        User_id, Password, dealer_id)
    cur.execute(q)
    con.commit()
    ...
    try:
        ...
        print("""
        <script>
        alert("Dealer approved and email sent successfully!");
        window.location.href = "approve.py?id=%s";
        </script>
        """ % (Id))
...
if rejsubmit !=None :
    ...
    a = """update dealer set status="rejected" where Id='%s' """ % (rejid)
    cur.execute(a)
    con.commit()

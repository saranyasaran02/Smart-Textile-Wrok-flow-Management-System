             ...
             <head>
                 ...
                 <style>
                     ...
                     .dealer-table {
                         ...
                         width: 100%;
                         background-color: white;
                         min-width: 1300px; /* Ensures table doesn't get too compressed */
                     }
    ...
    print("""
           <div class="content" id="content">
               <div class="page-container">
                   <h2 class="section-header">Dealer Approval Dashboard</h2>
                   <p class="text-muted mb-4">Review and approve pending dealer applications</p>
...
x = """select * from wholesaler where status='False' """
cur.execute(x)
res = cur.fetchall()
for i in res:
    whole_id = i[0]
    ...
    print("""
     ...
     <i class="fas fa-check-circle me-2"></i>Approve</button>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-header">
            <h4 class="modal-title">Approve Dealer: %s</h4>
          ...
          <div class="modal-body">
            <form method="post">
              <div class="form-group">
                ...
                <input type="text" class="form-control" name="user_id" value="%s" readonly>
              </div>
              <div class="form-group">
                <label class="form-label">Generated Password</label>
                <input type="text" class="form-control" name="password" value="%s" readonly>
              </div>
              <p class="text-muted mt-3">These credentials will be sent to the dealer's email upon approval.</p>
              <input type="hidden" name="email" value="%s">
              <input type="hidden" name="name" value="%s">
              <input type="hidden" name="whole_id" value="%s">
          </div>

          <!-- Modal footer -->
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            <input type="submit" class="btn btn-action" value="Confirm Approval" name="update">
            </form>
          </div>

        </div>
      </div>
    </div>""" % (whole_id,whole_id, i[1], user_id, password, i[3], i[1], whole_id))
    print("""

    <button type="button" class="btn btn-action" data-bs-toggle="modal" data-bs-target="#rejModal%s">
    <i class="fa-solid fa-circle-xmark"></i> Reject</button></td>
    </tr>
            <div class="modal" id="rejModal%s">
              <div class="modal-dialog">
                <div class="modal-content">

                   <!-- Modal Header -->
          <div class="modal-header">
            <h4 class="modal-title">Reject Dealer: %s</h4>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          ...
          <div class="modal-body">
            <form method="post">
              ...
              <p class="text-muted mt-3">Rejected upon invalid credentials.</p>
              <input type="hidden" name="email" value="%s">
              <input type="hidden" name="name" value="%s">
              <input type="hidden" name="rejid" value="%s">
          </div>

          <!-- Modal footer -->
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
            <input type="submit" class="btn btn-action" value="Reject" name="reject">
            </form>
          </div>
    ...
    """ % (whole_id,whole_id, i[1], i[3], i[1], whole_id))
...
whole= form.getvalue("whole_id")
Submit = form.getvalue("update")

if Submit is not None and whole is not None:

    q = """update wholesaler set username='%s', Password='%s', status="approved" where Id='%s' """ % (
        User_id, Password, whole)
    cur.execute(q)
    con.commit()
    ...
    try:
        ...
        print("""
        <script>
        alert("Wholesaler approved and email sent successfully!");
        window.location.href = "wapprove.py?id=%s";
        </script>
        """ % (Id))
...
if rejsubmit != None:
    ...
    a = """update wholesaler set status="rejected" where Id='%s' """ % (rejid)
    cur.execute(a)
    con.commit()
    ...
    try:
        ...
        print("""
        <script>
        alert("Wholesaler rejected and email sent successfully!");
        window.location.href = "wapprove.py?id=%s";
        </script>
        """ % (Id))
    except Exception as e:
